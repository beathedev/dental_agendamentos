...
$area_todos = $linhas33['FK_nomerota'];
    ...
    elseif(($quantidadeagend >= 1)&&($bloqueado1 == 0)){ 
    //desce camada 2
...
if($bloqueado1 == 1){
  ...
  <form action="consulta_result_rotinha.php" method="POST">
  ...
  if($ex_rotinha1 > 0){
  echo'<button type="submit" name="ae" class="btn btn-default">Ver agendendamentos em rotinha</button>
  </form>
  </center>';
}
  }
...
$PK_codrotav = $linhas33['PK_codrotav'];
    ...
    if($quantidadeagend < 1){   }
    elseif(($quantidadeagend >= 1) &&($bloqueado == 0)){ 
...
if($bloqueado == 1){
  ...
  <input type="hidden" name="dataentrega" value="'.$dataentrega.'">
  <input type="hidden" name="area" value="'.$area.'">';
  $sql_codrotava = "SELECT * from rotinha where FK_datarg='$dataentrega' and PK_codrotinha > 0";
  $query_rotava = mysqli_query($conexao,$sql_codrotava);
  $ex_rotinha = mysqli_num_rows($query_rotava);
  if($ex_rotinha > 0){
  echo'<button type="submit" name="ae" class="btn btn-default">Ver agendendamentos em rotinha</button>
  </form>
  </center>';
