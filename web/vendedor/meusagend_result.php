...
$bloqueado1 = $linhas['bloqueado'];
$FK_nomerota = $linhas['FK_nomerota'];
$rotinha = $linhas['FK_codrotinha'];
}
$verifica_autorizado = "SELECT * from agend where dataentrega = '$dataentrega' and autorizado ='Sim' and FK_coduser='$FK_coduser'";
$resultautoriza = mysqli_query($conexao, $verifica_autorizado); 
while($linhas_a = mysqli_fetch_array($resultautoriza)){
 $autorizado = $linhas_a['autorizado'];
}
...
//agendamentos autorizados pós bloqueio
if($autorizado == 'Sim'){   
//inicia consulta
...
$contador_rv = mysqli_num_rows($queryrota);
...
$sql_codrotav = "SELECT * from agend where dataentrega='$dataentrega' and FK_coduser = '$PK_coduser' and FK_codrotav='$PK_codrotav'and autorizado='Sim'";
$query_rotav = mysqli_query($conexao,$sql_codrotav);
$contador_todos=mysqli_num_rows($query_rotav);

    if($quantidadeagend < 1){   }
    elseif($quantidadeagend >= 1 and $contador_todos >= 1){ 
    //desce camada 2
...
<tr class="nomerota" >
 <th colspan="100%"><h4>
 <input type="text" name="FK_nomerota" class="cnomerota" value="'.$area_todos.'" readonly/>
 </h4></th></tr>';
...
if($conta_status > 0){
echo '<td><input type="hidden" value="1" >Efetivado</td>';/*name="status"*/
}  
else{
...
$sql_nomerota = "SELECT * from rotavend where FK_datarg='$PK_datarg' and FK_nomerota='$area'";
...
$sql_codrotav = "SELECT * from agend where FK_codrotav = '$PK_codrotav' and FK_coduser = '$PK_coduser' and dataentrega='$FK_datarg' and autorizado='Sim'";
$query_agend_esp = mysqli_query($conexao,$sql_codrotav);
$contador=mysqli_num_rows($query_agend_esp);



    if($quantidadeagend <= 0){    }
...
echo'</div>';
  ...
  echo'
  <form action="meusagends_result_rotinha.php" method="POST">
  <input type="hidden" name="dataentrega" value="'.$dataentrega.'">
  <input type="hidden" name="area" value="'.$area.'">';
  $sql_codrotava1 = "SELECT * from rotinha where FK_datarg='$dataentrega' and PK_codrotinha > 0";
  $query_rotava1 = mysqli_query($conexao,$sql_codrotava1);
  $ex_rotinha1 = mysqli_num_rows($query_rotava1);
  if($ex_rotinha1 > 0){
  echo'<button type="submit" name="consultar" class="btn btn-default">Ver agendendamentos em rotinha</button>
  </form>
  </center>';
}
...
// seleciona dados do agendamento da data de entrega
$sql_codrotav = "SELECT * from agend where dataentrega='$dataentrega' and FK_coduser = '$PK_coduser' and FK_codrotav='$PK_codrotav'";
$query_rotav = mysqli_query($conexao,$sql_codrotav);
$contador_todos=mysqli_num_rows($query_rotav);
...
$sql_codrotav = "SELECT * from agend where FK_codrotav = '$PK_codrotav' and FK_coduser = '$PK_coduser' and dataentrega='$FK_datarg'";
$query_agend_esp = mysqli_query($conexao,$sql_codrotav);
$contador=mysqli_num_rows($query_agend_esp);
