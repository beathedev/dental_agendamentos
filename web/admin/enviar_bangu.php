...
include_once("../conexao.php");
 $PK_codagend1 = $_POST['agends'];
    //Codigo agendamento 
  $codrotageral = $_POST['codrotageral'];
$select="SELECT * FROM rotageral WHERE codrotageral='$codrotageral'";
...
$qtdrotav = $recebe['qtdrotav'];
  ...
  $dia_seguinte=0;
  $data_termino = new DateTime($FK_datarg);
  ...
  ECHO $data_rota_layout;

$select2 = "SELECT * from rotavend where FK_datarg='$FK_datarg' and FK_nomerota='Bangu' OR FK_nomerota='bangu2' or FK_nomerota='bangu c'";
...
for ($i=0; $i < count($PK_codagend1) ; $i++) { 
  # code...
 $select3_ = "SELECT * from agend WHERE FK_codrotav = '$PK_codrotav' and PK_codagend='$PK_codagend1[$i]' ORDER BY cliente DESC";  //puxando agendamentos dessas rotas de vendedores
        $query3_ = mysqli_query($conexao, $select3_);
    $cont3_ = mysqli_num_rows($query3_);


    if($quantidadeagend < 1){   }
    elseif($quantidadeagend >= 1 and $cont3_ >0){ 
...
for ($i=0; $i < count($PK_codagend1) ; $i++) { 
  # code...
 $select3 = "SELECT * from agend WHERE FK_codrotav = '$PK_codrotav' and PK_codagend='$PK_codagend1[$i]' ORDER BY cliente DESC";  //puxando agendamentos dessas rotas de vendedores
