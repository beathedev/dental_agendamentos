...
ECHO'<TD><input type="checkbox" style="width:40px;" name="agend[]" value='.$PK_codagend.'></TD>
</tr>';


}// while
// comandos acima puxam dados da consulta pra exibiaçao dos agendamentos
...
for ($i=0; $i <count($PK_codagend) ; $i++) { 

$consultar = mysqli_query($conexao, "SELECT * FROM agend WHERE PK_codagend='$PK_codagend[$i]'");
while($tt = mysqli_fetch_array($consultar)){
$FK_coduser = $tt['FK_coduser'];
$dataentrega = $tt['dataentrega'];
...
$consultaaar = mysqli_query($conexao, "SELECT * FROM rotavend WHERE PK_codrotav='$FK_codrotav'");

while($ttt = mysqli_fetch_array($consultaaar)){
$area_old = $ttt['FK_nomerota'];


$consultar2 =  mysqli_query($conexao, "SELECT * FROM usuario WHERE PK_coduser='$FK_coduser'");
while($t1t = mysqli_fetch_array($consultar2)){

$vendedor = $t1t['username'];
  $dt2 = new DateTime($data_nova);
...
$upd1 =  mysqli_query($conexao, "UPDATE agend set FK_codrotav = '$FK_codrotav_nova', dataentrega='$data_nova' WHERE PK_codagend='$PK_codagend[$i]'");
...
';
    ...
    header("Refresh: 4s, url=lista_area.php");
...
$consultaaar = mysqli_query($conexao, "SELECT * FROM rotavend WHERE FK_codrotav='$FK_codrotav'");
while($ttt = mysqli_fetch_array($consultaaar)){
$area_old = $ttt['FK_nomerota'];


$consultar2 =  mysqli_query($conexao, "SELECT * FROM usuario WHERE PK_coduser='$FK_coduser'");
while($t1t = mysqli_fetch_array($consult2)){
$vendedor = $t1t['username'];
...
$upd3 =  mysqli_query($conexao, "UPDATE agend set FK_codrotav = '$FK_codrotav_nova', dataentrega='$data_nova' WHERE PK_codagend='$PK_codagend[$i]'");
}
