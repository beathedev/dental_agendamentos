...
for ($i=0; $i < count($array_obs) ; $i++) { 

$armazena[] = $array_obs[$i];
}

 $string = implode('', $armazena);
 $explode = explode('-', $string);

 $cont = count($array_obs);
for ($i=0; $i < count($explode) ; $i++) { 
if($obs == null){

echo 'Nenhuma forma de pagamento selecionada<br>';

echo '    <input type="radio" name="obs"  value="cheque">Cheque 
    </label>';
echo '<label>
...
echo '<label>
      <input type="radio" name="obs" checked value="cheque">Cheque 
    </label>';
...
echo '<label>
  ...
  }elseif($explode[$i] == "dinheiro"){

echo '      <input type="radio" name="obs"  value="cheque">Cheque 
    </label>';
echo '<label>
      <input type="radio" name="obs"  checked value="dinheiro">Dinheiro 
    </label>';
echo '<label>
      <input type="radio" name="obs"  value="boleto">Boleto 
    </label>';
...
echo '<label>
      <input type="radio" name="obs" checked value="boleto">Boleto 
    </label>';
echo '<label>
      <input type="radio" name="obs"  value="cartao">Cartão 
    </label>';
...
echo '<label>
      <input type="radio" name="obs"  checked value="cartao">Cartão 
    </label>';
echo '<label>
      <input type="radio" name="obs"  value="carteira">Carteira 
    </label>';
...
echo '<label>
      <input type="radio" name="obs" checked value="carteira">Carteira 
    </label>';
  } 

elseif($explode[$i] == "notafiscal"){
  echo' <br>
  <label for="exampleInputEmail1">Observação</label>';
  echo' <br>   <label>
      <input type="radio" name="obs_nf" checked value="notafiscal"> Nota Fiscal     
    </label>';
}
...
?>
 ...
 <br><br>
  <label>Outros
<div id="text_area3">
<TEXTAREA name="outros"><?php echo $outros ?></TEXTAREA> 
</div>
</label>
...
echo $PK_codped2 = $explode[0];
$ped = $explode[1];
$val = $explode[2];
echo $PK_codagend2 = $explode[3];
...
//campo CAIXAO
if($status == "Possui Pedido"){
echo '<p><input type="hidden" value="1" name="status">
<label>
Clique aqui para tornar Caixão:
<input type="checkbox" value="0" name="status">
  ...
  echo '<p><input type="hidden" value="1" name="status">
<label>
Clique aqui para deixar de ser Caixão:
...
// AQ deleta o ped de acordo com o agendamento
...
$selectt =mysqli_query($conexao, "SELECT * from agend where FK_codrotav='$PK_codrotav'");
...
$sql_atualiza = "UPDATE agend set FK_coduser='$FK_coduser', obs='$obs', volume='$volume', dataabertura='$dataabertura', dataentrega='$dataentrega', cliente='$cliente', endereco='$endereco', bairro='$bairro', status='$status', almoco='$almoco', outros='$outros' WHERE dataentrega = '$dataentrega' and PK_codagend =  '$PK_codagend'  ";
...
// $sql_atualizal = "UPDATE agend set status = 1 WHERE PK_codagend = '$PK_codagend' ";
...
//  $sql_atualizal2 = "UPDATE agend set status = 1 WHERE PK_codagend = '$PK_codagend' ";
// $query_atualizal2 = mysqli_query($conexao, $sql_atualizal2);
