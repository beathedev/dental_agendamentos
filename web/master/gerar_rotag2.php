...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // $data_rota = filter_input(INPUT_POST, 'ddata_rotaata');
...
}ECHO "DATA: ", $data_rota_layout;
echo'<input type="HIDDEN" name="data_rota" value="'.$data_rota.'" readonly/><br><br>';
...
$qtdrotav = $linhas1['qtdrotav']; //desce camada 1
 $sql2 = "SELECT * from rotavend WHERE FK_datarg = '$PK_datarg'  ORDER BY entregador ASC";  
...
//echo 'consulta inicial:'.$transporte.'<br>';
...
//Select agendamentos
        ...
        $sql3 = "SELECT * from agend WHERE FK_codrotav = '$PK_codrotav' ORDER BY PK_codagend ASC";  //puxando agendamentos dessas rotas de vendedores
...
echo '<td style="background-color:black;width:1px;"></td>'; 
             ...
             echo'
            <td>'.$cliente.'</td>
            <td> '.$obs.'  <p style="color:red;"> '.$outros.'</p></td>
            <td>'.$hr_almoco.'</td>
            <td>'.$endereco.'</td>
            <td>'.$bairro.'</td>';
...
$contae = mysqli_num_rows($queryp1);
                            if ($contae<1){
                              echo '<td style="background-color:yellow">Caixao';
                            }else{
                              echo '<td>'.$contae;
                            }
echo'
...
$descricao = "deletou o seguinte agendamento da rota geral do dia $: - Cliente: ".$nomecliente." - Vendedor: ".$vendedor." - Endereço: ".$endereco;
...
$turnoA = $_POST['turnoA'];
        ...
        // CF NÃO ALTERAR SOB NENHUMA CIRCUNSTANCIA
        $cfd = $_POST['cfd'];
...
if($entregadorA == "Escolher Entregador"){
 echo 'consulta aqui1:'.$transporteA.'<br>';
...
echo 'consulta aqui2:'.$transporteA.'<br>';
...
$insert_novarota_ = "INSERT INTO rotavend(FK_datarg, FK_nomerota, quantidadeagend, bloqueado, limiteagend, turno, tipo, entregador) VALUES('$FK_datarg', '$FK_nomerota_', '1', '1', '7', '$turnoA', 'tarde','$entregadorA')";
...
$descricao = "definiu as seguintes alterações na rota geral do dia ".$data_." na área ".$FK_nomerota_."
 Informações do agendamento: - Entregador: ".$entregador_old." - Transporte: ".$transporte_old." - Turno: ".$turno_old.", Alteradas para:  - Entregador: ".$entregadorA." - Transporte: ".$transporteA2." - Turno: ".$turnoA;
}elseif($transporteA == 1){
