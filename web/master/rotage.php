...
echo'<input type="HIDDEN" name="" value="'.$data_rota.'" readonly/><br><br>';

$sql1 = "SELECT * from rotageral WHERE PK_datarg = '2019-06-15'";  //puxando rota geral pela data 
...
//bptao filtro
echo'
<form action="rotage.php" method="POST">
<input type="HIDDEN" value="'.$PK_datarg.'" name="PK_datarg"> 
<input type="HIDDEN" value="'.$codrotageral.'" name="codrotageral">
...
echo'</select>  <input type="submit" name="visualizar" class="btn btn-default" value="Visualizar"></div></label> </form>';
//botao filtro termina aq
...
<input type="HIDDEN" value="'.$codrotageral.'" name="codrotageral">
<div style="style="background-color:white;width:40%; height:50%;">
<label>Filtrar (por entregador - Impressao)
<select name="entregador_imprimir">
<option value="Todos">Todos</option>
...
echo'</select><input type="submit" value="Imprimir" name="imprimiu" class="btn btn-default" style="margin-left:10px;"></div></label></form>
';
...
//filtro
if($motorista != "Todos"){
   $sql2 = "SELECT * from rotavend WHERE FK_datarg='$PK_datarg' and entregador = '$motorista' ORDER BY entregador ASC";
...
echo '<td style="background-color:black;width:1px;"></td>'; 
             ...
             echo'
            <td>'.$cliente.'</td>
            <td> '.$obs.'  <div style="background-color:red;"> '.$outros.'</div></td>
            <td>'.$hr_almoco.'</td>
            <td>'.$endereco.'</td>
            <td>'.$bairro.'</td>';
...
$contae = mysqli_num_rows($queryp1);
                          ...
                          if ($contae<1){
							 ...
							 echo '<td style="background-color:yellow">Caixao 2';
							}elseif($status == 1){
								   echo '<td>'.$contae;
							}else{
                              echo '<td>'.$contae;
