...
<center><p>Selecione abaixo os agendamentos que deseja imprimir:</p>
<input type="submit"  name="envio" id="botao" onclick="altera_form();" value="Enviar" />
    <br>
    <br>
...
$select2 = "SELECT * from rotavend where FK_datarg='$FK_datarg' and FK_nomerota='Bangu' or  FK_datarg='$FK_datarg' and FK_nomerota='bangu2' or FK_datarg='$FK_datarg' and FK_nomerota='bangu C' order by entregador asc";
...
<thead>
<tr class="nomerota" >
<input type="hidden" name="codrotageral" value="'.$codrotageral.'"/>
<input type="hidden" name="FK_datarg" value="'.$FK_datarg.'"/>
...
';
 ...
 $select3 = "SELECT * from agend WHERE FK_codrotav = '$PK_codrotav' ORDER BY bairro DESC";  //puxando agendamentos dessas rotas de vendedores
...
echo'</td>
<td><input type="checkbox" name="agends[]" value="'.$PK_codagend.'"></td>
